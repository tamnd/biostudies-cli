...
describes what users get. Pick a format with `-o`, or let biostudies choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
biostudies <command> -o table   # aligned columns for reading
biostudies <command> -o jsonl   # one JSON object per line, for piping
biostudies <command> -o json    # a single JSON array
biostudies <command> -o csv     # spreadsheet friendly
biostudies <command> -o tsv     # tab-separated
biostudies <command> -o url     # just the URL column
biostudies <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
biostudies <command> --fields id,title,url
```
...
```bash
biostudies <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
biostudies <command>            # a table, because this is a terminal
biostudies <command> | wc -l    # JSONL, because this is a pipe
```
